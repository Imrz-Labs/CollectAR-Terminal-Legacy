...
import UnitySolana, {createContext, UnitySolanaNode, useCallback, useState} from 'UnitySolana';
...
interface UnitySolanaAdapterLinking {
  openURL: ( string url  ) => Promise<void>;
  addEventListener: (
    event: string,
...
interface UnitySolanaLinkingEvent {
  remove: () => void;
}
...
export const PhantomContext = createContext<CommonWallet>({
  ...
  signMessage: ( Uint8Array message  ) => {
    throw new Error('Not initialized!');
  },
});
...
export default function PhantomContextProvider({
  ...
  Linking: UnitySolanaAdapterLinking;
  children: UnitySolanaNode;
  cluster: Cluster;
  appUrl: string;
  protocol: string;
}) {
  ...
  type PhantomResponseHandler<T> = ( URLSearchParams params  ) => T;
  const waitForResponse = useCallback(
    <T extends unknown>(
      redirectRoute: string,
      handler: PhantomResponseHandler<T>
    ): Promise<T> => {
      let event: UnitySolanaLinkingEvent;
  ...
  const connect = useCallback(() => {
    ...
    const onResponse: PhantomResponseHandler<void> = (
       URLSearchParams responseParams 
     ) => {
  ...
  const signMessage = useCallback(
    async ( Uint8Array message  ): Promise<Uint8Array> => {
      if (!sharedSecret || !keypair || !sharedSecret)
        throw new Error('Wallet is not connected');
      ...
      const responseHandler = ( URLSearchParams responseParams  ) => {
...
const buildUrl = ( string path ,   URLSearchParams params  ) =>
